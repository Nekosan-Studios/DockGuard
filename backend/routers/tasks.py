...
@router.get("/tasks")
def get_recent_tasks(limit: int = Query(default=100, le=500), session: Session = Depends(db.get_session)):
    """Get the recent history of background tasks (scheduled jobs, scans)."""
    tasks = session.exec(select(SystemTask).order_by(SystemTask.created_at.desc()).limit(limit)).all()
