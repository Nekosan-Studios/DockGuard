<script lang="ts">
  ...
  let tooltipText = $derived.by(() => {
    ...
    if (vexStatus === "not_affected")
      parts.push("Supplier declares: Not Affected");
    else if (vexStatus === "affected")
      parts.push("Supplier declares: Affected");
    else if (vexStatus === "under_investigation")
      parts.push("Supplier: Under Investigation");
...
<Table.Cell class="text-center {className}">
  ...
  {:else if vexStatus === "affected"}
    <Tooltip.Provider>
      <Tooltip.Root>
        <Tooltip.Trigger class="cursor-default">
          <ShieldAlert class="mx-auto h-4 w-4 text-red-600 dark:text-red-400" />
        </Tooltip.Trigger>
        <Tooltip.Content class="max-w-xs whitespace-pre-line">
          {tooltipText}
        </Tooltip.Content>
  ...
  {:else if vexStatus === "under_investigation"}
    <Tooltip.Provider>
      <Tooltip.Root>
        <Tooltip.Trigger class="cursor-default">
          <Clock class="mx-auto h-4 w-4 text-amber-600 dark:text-amber-400" />
        </Tooltip.Trigger>
        <Tooltip.Content class="max-w-xs whitespace-pre-line">
          {tooltipText}
        </Tooltip.Content>
