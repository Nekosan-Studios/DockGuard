...
describe("VexStatusCell", () => {
  it("renders a dash for null/unknown statuses", () => {
    ...
    const { container: container2 } = render(VexStatusCell, {
      vexStatus: "random_status",
    });
  ...
  it("builds tooltip string with statements when provided", async () => {
    ...
    expect(
      await screen.findByText(/Supplier declares: Not Affected/i)
    ).toBeInTheDocument();
    expect(
      await screen.findByText(/vulnerable_code_not_in_execute_path/i)
    ).toBeInTheDocument();
    expect(
      await screen.findByText(/We stripped the vulnerable binary/i)
    ).toBeInTheDocument();
  });
});
