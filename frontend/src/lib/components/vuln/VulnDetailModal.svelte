        ...
        <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <div class="rounded-lg border p-3">
            ...
            {#if vuln.cvss_base_score != null}
              <div class="text-xl font-bold {cvssClass(vuln.cvss_base_score)}">
            ...
            <div class="rounded-lg border p-3">
              ...
              {#if vuln.match_type === "exact-indirect-match" && vuln.upstream_name}
                <div class="text-muted-foreground mt-1 text-xs">
                  ...
                  <span class="font-mono font-medium">{vuln.upstream_name}</span
                  >
                </div>
              {/if}
              {#if vuln.purl}
                  ...
                  <ul class="mt-0.5 space-y-0.5">
                    {#each paths as path (path)}
                      <li class="font-mono text-xs break-all">
                        <span class="text-muted-foreground mr-1">•</span>{path}
                      </li>
                    {/each}
                  </ul>
                </div>
